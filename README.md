...
Каждый проект на машине автоматически получает **свою** идентичность пира — отдельную ed25519-пару, отдельный machine id на mediator, отдельные контакты и имя. Ключ привязки — путь к каталогу проекта: идентичность лежит в `~/.config/c2c-client/projects/<slug-пути>/`, где `<slug>` = `<имя-папки>-<хэш-полного-пути>`. Разные проекты = разные пиры; переименование/перемещение папки = новый пир (контакты придётся завести заново). `mediator url`/`token` при этом **общие** для всех проектов (лежат в `~/.config/c2c-client/config.json`), поэтому `/c2c-client:peer-config` достаточно выполнить один раз на машину.
