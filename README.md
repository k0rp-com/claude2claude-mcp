...
На одну идентичность (проект) держится **ровно один** listener — второй гонялся бы за тем же inbox и дублировал доставку. `listener.pid` хранит `PID SESSION_ID` владельца (`CLAUDE_CODE_SESSION_ID`). Поэтому листенер, перенесённый через `/clear` в той же сессии, не поднимается повторно, а вот запуск `/c2c-client:peer-listen` (или авто-арм на старте) в **другой** сессии **перехватывает** listener себе: старый (в т.ч. осиротевший от закрытой сессии) корректно останавливается TERM→KILL и inbox начинает слушать текущая сессия. Регрессия: `tests/client/listener-takeover.test.ts`.
