...
describe('cleanup — unacked TTL', () => {
  it('deletes unacked messages older than unackedTtlMs, keeps fresh ones', () => {
    ...
    db.registerMachine({ id: 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', pubkeyPem: pk(), name: 'alice' });
    db.registerMachine({ id: 'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb', pubkeyPem: pk(), name: 'bob' });
    db.createPairing('aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', 'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb');

    const fresh = db.send({ fromId: 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', toId: 'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb', kind: 'request', body: 'fresh' });
    const old = db.send({ fromId: 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', toId: 'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb', kind: 'request', body: 'stale' });
    db.close();
    ...
    expect(db2.countUnacked('bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb')).toBe(1);
    // Fresh message still retrievable.
    const inbox = db2.inbox('bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb', 0);
  ...
  it('startCleanup wires unackedTtlMs into the sweep', () => {
    ...
    db.registerMachine({ id: 'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb', pubkeyPem: pk(), name: 'bob' });
    db.createPairing('aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', 'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb');
    db.send({ fromId: 'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa', toId: 'bbbbbbbb-bbbb-4bbb-8bbb-bbbbbbbbbbbb', kind: 'request', body: 'hi' });
