...
- **Single-listener mutex + session takeover** (`common.sh` → `c2c::listener_*`). Only one listener may run per identity (a second races the same unacked inbox → double delivery). `listener.pid` stores `PID SESSION_ID` where owner = `CLAUDE_CODE_SESSION_ID` (inherited by the Monitor-spawned `listen.sh` and the SessionStart hook). `c2c::listener_state` returns `none|dead|mine|foreign`: `mine` (same session, e.g. Monitor carried across `/clear`) → don't start a second; `foreign` (another/closed/orphaned session) → `listen.sh` **takes it over** via `c2c::listener_takeover` (TERM→wait-for-death→KILL) then `listener_claim`s — so arming in the current session always wins. `_pid_is_listener` (`ps -o args= | grep listen.sh`) guards PID reuse so a recycled PID is never signalled; empty `CLAUDE_CODE_SESSION_ID` falls back to "never kill a live listener" (old behavior). `listen.sh` traps `INT/TERM` to `rm` its pid file **and exit** (cooperative stop for fast handoff). Regression: `tests/client/listener-takeover.test.ts` (helpers + real-`listen.sh` integration).
